...
def record_entropy(file_name):
    with open(file_name, 'r', encoding='utf-8') as f:
        ...
        for line in content:
            ...
            for characer in line:
                ...
                if (content_len / record_delta) > record_num:
                    ...
                    with open("entropy_en.txt", "a") as fw:
                        write_str = str((content_len / record_delta) / 1024) + ":" + str(current_entropy) + "\n"
                        fw.write(write_str)
                        print("recorded file size : ", (content_len / record_delta)  / 1024, " MB")
        ...
        print('语料库共有{0}个字母'.format(content_len))
        print('其中有{0}个不同的字母'.format(len(characers)))
        ...
        current_entropy = calculate_entropy(rate, content_len)
        with open("entropy_en.txt", "a") as fw:
            write_str = str((content_len / 1024) / 1024) + ":" + str(current_entropy) + "\n"
            fw.write(write_str)
...
if __name__ == "__main__":
    ...
    entropy = record_entropy("en_content.txt")
